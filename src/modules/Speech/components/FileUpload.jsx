  ...
  (
    ...
    const validateFile = (file) => {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        return `${
          t('speech.unsupportedType') || 'Unsupported file type. Allowed:'
    ...
    const startRecording = useCallback(async () => {
      try {
        ...
        onError?.(
          t('speech.micError') || 'Could not access microphone. Please check permissions.'
        );
      }
    }, [onError, t]);
    ...
    const performAnalysis = async (fileToUpload, ageToUse) => {
      try {
        const result = await analyzeVoiceAudio(fileToUpload, ageToUse);
        ...
        sessionStorage.setItem(
          'analysisError',
          error.message || t('speech.analysisFailure') || 'Failed to analyze audio. Please try again.'
        );
    ...
    const handleSubmit = async (fileOverride = null, ageOverride = null) => {
      const fileToUpload = fileOverride || selectedFile;
      const ageToUse = ageOverride ?? childAge;

      if (!fileToUpload) {
        onError?.(
          t('speech.selectOrRecord') || 'Please select an audio file or record audio first'
        );
        return;
      }
      ...
      sessionStorage.setItem('pendingChildAge', String(ageToUse));
      sessionStorage.removeItem('analysisResult');
      sessionStorage.removeItem('analysisError');

      navigate('/speech/results');
      performAnalysis(fileToUpload, ageToUse);
    };
    ...
    useImperativeHandle(ref, () => ({
      handleAutoAnalyze: (file, age) => {
        ...
        setTimeout(() => {
          handleSubmit(file, finalAge);
        }, 100);
      },
    }));

    useEffect(() => {
      if (autoAnalyze && initialFile && location.pathname === '/speech/analyze') {
        const timer = setTimeout(() => {
          handleSubmit(initialFile, initialChildAge ?? childAge);
        }, 500);

        return () => clearTimeout(timer);
      }
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [autoAnalyze, initialFile, initialChildAge, location.pathname]);
    ...
    return (
      <div className="space-y-8">
        ...
        {mode === 'upload' && (
          <div
            className={`relative border-2 border-dashed rounded-[2rem] p-12 text-center transition-all duration-300 group ${
              dragActive
                ? 'border-blue-400 bg-blue-500/20 scale-105 shadow-[0_0_30px_rgba(59,130,246,0.3)]'
                : selectedFile
                ? 'border-green-400 bg-green-500/10 shadow-[0_0_20px_rgba(74,222,128,0.2)]'
                : 'border-white/30 hover:border-blue-400/50 hover:bg-white/5 hover:shadow-[0_0_20px_rgba(59,130,246,0.15)] cursor-pointer'
            }`}
            ...
            <div className="space-y-6 relative z-10">
              {selectedFile ? (
                <div className="animate-fade-in-up">
                  ...
                  <div>
                    <h3 className="text-3xl font-black text-white mb-2 drop-shadow-sm">
                      {selectedFile.name}
                    </h3>
                    <p className="text-green-300 font-medium mb-6 drop-shadow-sm">
                ...
                <div className="animate-fade-in-up group-hover:transform group-hover:-translate-y-2 transition-transform duration-300">
                  <div className="inline-flex items-center justify-center w-24 h-24 bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl mb-8 group-hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] group-hover:bg-blue-500/20 transition-all duration-300">
                    <svg
                      className="w-12 h-12 text-white/70 group-hover:text-blue-400 transition-colors duration-300"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <div>
                    <h3 className="text-3xl font-black text-white mb-4 tracking-wide group-hover:text-blue-200 transition-colors duration-300">
                      {t('speech.dropAudio')}
                    </h3>
                    <p className="text-white/70 font-medium mb-8 max-w-md mx-auto leading-relaxed">
                      {t('speech.dropAudioDesc')}
                    </p>
                    <div className="flex flex-wrap justify-center gap-3">
                      {allowedExtensions.map((ext, index) => (
                        <span
                          key={index}
                          className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-white/60 tracking-wider group-hover:border-white/20 group-hover:bg-white/10 transition-colors duration-300"
                        >
                          {ext.toUpperCase()}
                        </span>
                      ))}
            ...
            <div className="space-y-8 relative z-10">
              {selectedFile && recordedBlob ? (
                <div className="animate-fade-in-up">
                  ...
                  <div>
                    <h3 className="text-3xl font-black text-white mb-2 drop-shadow-sm">
                      {t('speech.recordingSaved')}
                    </h3>
                    <p className="text-green-300 font-medium tracking-wider mb-8 drop-shadow-sm">
                      {t('speech.duration')}: {formatTime(recordingTime)}
                    </p>
                    <button
                      onClick={clearFile}
                      className="px-6 py-2 rounded-full border border-white/20 hover:bg-white/10 text-white/80 hover:text-white transition-all duration-300 text-sm font-bold tracking-wider"
                    >
                ...
                <div className="animate-fade-in-up">
                  <div
                    className={`relative inline-flex items-center justify-center w-32 h-32 rounded-full mb-8 transition-all duration-300 ${
                      isRecording
                  ...
                  >
                    ...
                    <svg
                      className={`w-14 h-14 ${
                        isRecording ? 'text-white drop-shadow-md' : 'text-white/70'
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                  ...
                  <div>
                    <h3 className="text-3xl font-black text-white mb-6 drop-shadow-md">
                      {isRecording
                        ? t('speech.recordingStatus')
                        : t('speech.clickToRecord')}
                    </h3>

                    {isRecording && (
                      <div className="mb-8 p-6 bg-black/20 rounded-2xl border border-red-500/30 backdrop-blur-md">
                        <div className="text-5xl font-mono font-black text-red-400 mb-4 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)] tracking-wider">
                    ...
                    <p className="text-white/70 font-medium mb-10 max-w-md mx-auto leading-relaxed">
                      {isRecording
                        ? t('speech.recordingTip')
                        : t('speech.recordPermTip')}
                    </p>
                    ...
                    <button
                      ...
                      className={`px-10 py-5 rounded-full font-black text-lg uppercase tracking-wider transition-all duration-300 transform ${
                        isRecording
                          ? 'bg-gradient-to-r from-red-600 to-red-500 text-white shadow-[0_0_30px_rgba(239,68,68,0.5)] hover:scale-110'
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-[2.5rem] p-10 border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] relative overflow-hidden group/age">
          ...
          <div className="max-w-md mx-auto relative z-10">
            <label
              htmlFor="childAge"
              className="block text-sm font-bold text-white/40 uppercase tracking-[0.2em] mb-4 text-center"
            >
              {t('speech.childAge')}
            </label>
              ...
              >
                {[6, 7, 8, 9, 10].map((age) => (
                  <option
                    key={age}
                    value={age}
                    className="text-lg bg-[#0E1B4D] font-bold py-4"
                  >
            ...
            <p className="text-xs font-bold text-white/30 mt-6 text-center tracking-widest uppercase">
              {t('speech.ageNote')}
            </p>
          </div>
        </div>
        ...
        <div className="text-center mt-12 animate-fade-in-up">
          <button
            ...
            className={`relative overflow-hidden group px-14 py-6 rounded-full font-black text-xl uppercase tracking-widest transition-all duration-300 transform ${
              selectedFile
                ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:scale-105 shadow-[0_0_30px_rgba(59,130,246,0.5)] border border-blue-400/50 hover:shadow-[0_0_40px_rgba(168,85,247,0.7)]'
          ...
          >
            ...
            <span className="relative z-10 drop-shadow-md">
              {selectedFile
                ? t('speech.analyzeBtn')
                : mode === 'upload'
                ? t('speech.selectFileFirst')
                : t('speech.recordFirst')}
            </span>
          </button>
        </div>
      </div>

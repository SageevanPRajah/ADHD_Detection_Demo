...
export default function Game2FruitCatching({ durationMs, onEvent, onDone }) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            if (activeFruit && !activeFruit.caught) {
                ...
                const s = 84;
                if (img.complete)
                    ctx.drawImage(img, activeFruit.x - s / 2, activeFruit.y - s / 2, s, s);
            }
        ...
        rafId = requestAnimationFrame(draw);
        const onClick = (e) => {
            if (finished || !activeFruit || activeFruit.caught)
                return;

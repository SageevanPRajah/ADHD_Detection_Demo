...
function points9(w, h) {
    const xs = [0.15, 0.5, 0.85], ys = [0.2, 0.5, 0.8];
    const pts = [];
    for (const y of ys)
        for (const x of xs)
            pts.push({ x: Math.round(x * w), y: Math.round(y * h) });
    return pts;
}
function points3(w, h) {
 ...
 */
export default function Calibration({ onEvent, onCalibSample, getLatestGaze, onModel }) {
    ...
    useEffect(() => {
        ...
        const DOT_MS = 1400;
        const HOLD_MS = 450;
        const getPts = (w, h) => (mode === 'calib9' ? points9(w, h) : points3(w, h));
        const draw = () => {

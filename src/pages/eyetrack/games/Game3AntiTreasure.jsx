...
export default function Game3AntiTreasure({ durationMs, onEvent, onDone }) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            if (rover.complete)
                ctx.drawImage(rover, roverPos.x - 110, roverPos.y - 80, 220, 160);
            if (blastOnRef.current && blast.complete)
                ctx.drawImage(blast, roverPos.x - 90, roverPos.y - 110, 180, 180);
            ...
            if (phase === 'resp') {
                ...
                if (treasure.complete)
                    ctx.drawImage(treasure, p.x - 62, p.y - 62, 124, 124);
                if (t >= nextAt) {
